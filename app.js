(() => {
  ...
  const APP_VERSION = 'Domácnost+ v.0.1_431';
  const APP_BUILD = 431;
  ...
  const DEFAULT_FINANCE_TEMPLATES = [
    { id: 'salary', icon: '💼', name: 'Výplata', type: 'income', title: 'Výplata', category: 'salary', paymentMethod: 'bank_transfer', amount: '', note: '', system: true },
    { id: 'rent', icon: '🏠', name: 'Nájem', type: 'expense', title: 'Nájem', category: 'housing', paymentMethod: 'bank_transfer', amount: '', note: '', system: true },
    { id: 'electricity', icon: '⚡', name: 'Elektřina', type: 'expense', title: 'Elektřina', category: 'energy', paymentMethod: 'direct_debit', amount: '', note: '', system: true },
    { id: 'gas', icon: '🔥', name: 'Plyn', type: 'expense', title: 'Plyn', category: 'energy', paymentMethod: 'direct_debit', amount: '', note: '', system: true },
    { id: 'water', icon: '💧', name: 'Voda', type: 'expense', title: 'Voda', category: 'housing', paymentMethod: 'direct_debit', amount: '', note: '', system: true },
    { id: 'cash', icon: '💵', name: 'Výběr hotově', type: 'expense', title: 'Výběr hotově', category: 'other_expense', paymentMethod: 'cash', amount: '', note: '', system: true },
    { id: 'savings', icon: '🏦', name: 'Na spoření', type: 'transfer', title: 'Přesun na spoření', category: 'other_expense', paymentMethod: 'bank_transfer', amount: '', note: '', system: true }
  ];

  const SUBSCRIPTION_SERVICE_OPTIONS = [
    ['netflix', 'Netflix', 319, 5],
    ['disney', 'Disney+', 199, 7],
    ['hbo', 'HBO Max', 219, 5],
    ['skyshowtime', 'SkyShowtime', 179, 5],
    ['prime', 'Prime Video', 79, 6],
    ['apple-tv', 'Apple TV+', 199, 6],
    ['spotify', 'Spotify', 229, 6],
    ['youtube', 'YouTube Premium', 239, 6],
    ['oneplay', 'Oneplay', 199, 5],
    ['canal', 'Canal+', 189, 4],
    ['o2-tv', 'O2 TV', 399, 4],
    ['telly', 'Telly', 250, 4],
  ...
  const ASSET_ICON_THEMES = {
    ...
    'isometric-micro': { surface: 'dark', pathPrefix: './icons/icon-themes/isometric-micro/' }
  };
  ...
  let shoppingRuntimeReady = false;
  ...
  // pwaControllerReloadTriggered). Instance dostupná přes getPwaModule().
  ...
  // Akumulace zdrojů (tabulek), které přišly během jednoho debounce okna.
  // Set zajišťuje dedup a přežije trailing debounce reset timeru — do jednoho
  // refreshe se zpracují všechny nashromážděné zdroje najednou.
  ...
  function migrateState(input, options = {}) {
    ...
    migrated.profiles = dedupeProfiles(migrated.profiles, { activeProfileId: migrated.activeProfileId, householdId: migrated.household.id, stateSnapshot: migrated });
  ...
  function applyVisualSettings() {
    ...
    const signature = `${state.settings.theme}|${state.settings.iconTheme}|${state.settings.colorScheme}`;
    if (signature === lastVisualSettingsSignature) return;
    lastVisualSettingsSignature = signature;
  ...
  function profileDedupeKey(profile = {}, options = {}) {
    const snapshot = options.stateSnapshot || runtimeStateRef || {};
    const householdId = normalizeText(profile.householdId || profile.household_id || options.householdId || snapshot.household?.id || snapshot.cloud?.householdId || '');
    const name = normalizeKey(profile.name || profile.displayName || '');
    if (name) return `household:${householdId || 'current'}:name:${name}`;
  ...
  function profileDedupeScore(profile = {}, activeProfileId = '', options = {}) {
    const snapshot = options.stateSnapshot || runtimeStateRef || {};
    let score = 0;
    ...
    if (profile.userId && profile.userId === snapshot.cloud?.userId) score += 30;
  ...
  function dedupeProfiles(profiles = [], options = {}) {
    const snapshot = options.stateSnapshot || runtimeStateRef || {};
    const activeProfileId = options.activeProfileId || snapshot.activeProfileId || '';
    const householdId = options.householdId || snapshot.household?.id || snapshot.cloud?.householdId || '';
    const byKey = new Map();
    const order = [];
    (Array.isArray(profiles) ? profiles : []).forEach((profile, index) => {
      ...
      const key = profileDedupeKey(normalized, { ...options, householdId, stateSnapshot: snapshot });
      ...
      const keepNext = profileDedupeScore(normalized, activeProfileId, { ...options, stateSnapshot: snapshot }) > profileDedupeScore(existing, activeProfileId, { ...options, stateSnapshot: snapshot });
  ...
  function renderContractOverviewItem(contract) {
    return getContractsModule().renderContractOverviewItem(contract);
  }
  ...
  function renderTaskOverviewItem(task) {
    return getNotesModule().renderTaskOverviewItem(task);
  }
  ...
  function renderHomeHeroSummaryItems(ctx) {
    ...
    return selected.map((id) => {
      ...
      const extraRows = Array.isArray(presentation.extraRows) && presentation.extraRows.length
        ...
        : '';
      const commonInner = `
          ${renderHomePanelIcon(id, { density, label: item.label })}
  ...
  function renderHomePanelIcon(id, options = {}) {
  ...
  function getHomeHeroItemPresentation(id, ctx, item, metric, text, options = {}) {
    ...
    if (id === 'loyaltyCards') {
      ...
      const currentCard = homeCycleItem(cards, 45);
      const favoriteCount = cards.filter((entry) => entry.favorite).length;
      const typeLabel = currentCard?.codeType === 'qr' ? 'QR kód' : currentCard?.codeType === 'text' ? 'textový kód' : 'čárový kód';
      return {
        ...base,
        metric: cards.length,
        text: currentCard ? firstTitle(currentCard, 'Karta') : 'Žádná karta',
        detail: currentCard ? typeLabel : 'Přidej kartu v Nákupy → Karty',
        chips: cards.slice(0, density === 'large' ? 4 : 2).map((entry) => entry.store).filter(Boolean),
        live: cards.length > 1,
    ...
    if (id === 'warranties') {
      ...
      const hasWarrantyDays = Boolean(currentWarranty) && currentWarranty.days !== null && currentWarranty.days !== undefined;
      return {
        ...base,
        metric: warranties.length,
        text: currentWarranty ? firstTitle(currentWarranty, 'Věc') : 'Žádná záruka',
        detail: hasWarrantyDays ? `do ${dueBadge(currentWarranty.days)}` : currentWarranty ? 'bez konce záruky' : 'Přidej koupenou věc',
        live: warranties.length > 1,
        tone: hasWarrantyDays && currentWarranty.days <= 30 ? 'warn' : warranties.length ? 'good' : 'neutral'
      };
    }
    if (id === 'garage') {
      const alert = (ctx.vehicleAlerts || [])[0];
  ...
  function rememberIconHtml(key, html) {
    if (iconHtmlCache.has(key)) return iconHtmlCache.get(key);
    iconHtmlCache.set(key, html);
    if (iconHtmlCache.size > ICON_HTML_CACHE_LIMIT) {
      const firstKey = iconHtmlCache.keys().next().value;
      iconHtmlCache.delete(firstKey);
    }
    return html;
  }
  ...
  function getModuleIllustrationSrc(id) {
    return `./icons/module-icons/${getModuleIllustrationId(id)}.png`;
  }
  ...
  function themedIconColors(id) {
    return {
      ...
      loyaltyCards: ['#EC4899', '#FCE7F3', '#111827'],
  ...
  function renderWeatherAnimeIcon(code, options = {}) { return getWeatherModule().renderWeatherAnimeIcon(code, options); }
  ...
  function weatherCodeLabel(code) { return getWeatherModule().weatherCodeLabel(code); }
  ...
  function getCloudLocalSyncAudit() {
    const overview = getCloudSyncOverviewItems();
    const online = overview.filter((item) => item.total && !item.local);
    const localOnly = overview.filter((item) => item.local);
    const watched = ['Nákupy', 'Předplatné', 'Garáž', 'Záruky', 'Smlouvy', 'HDO', 'Kalendář', 'Svátky PL', 'Počasí', 'Nastavení vzhledu'];
    const riskNotes = [];
  ...
  function renderCloudLocalSyncAudit() {
    const audit = getCloudLocalSyncAudit();
    const onlineNames = audit.online.map((item) => item.label).slice(0, 10);
    const localNames = audit.localOnly.map((item) => `${item.label} (${item.local})`).slice(0, 10);
    return `
      <section class="card desktop-span-2 cloud-sync-audit-card">
        <div class="card-header">
          <div><h2>Audit cloud / lokál</h2><p>Rychlá kontrola, co je online synchronizované a co je ještě jen v tomto zařízení.</p></div>
          <span class="badge ${audit.localOnly.length ? 'warn' : 'good'}">${audit.localOnly.length ? `${audit.localOnly.length} ke kontrole` : 'čisté'}</span>
        </div>
        <div class="cloud-status-grid compact-cloud-stats">
          <div class="mini-stat"><span>Online OK</span><strong>${audit.online.length}</strong></div>
          <div class="mini-stat"><span>Lokální / čeká</span><strong>${audit.localOnly.length}</strong></div>
          <div class="mini-stat"><span>Sledované směry</span><strong>${audit.watched.length}</strong></div>
        </div>
        <div class="setup-list compact-sync-audit-list">
          <div class="setup-item done"><span>✓</span><div><strong>Online sync hotovo / bez lokálních čekajících dat</strong><em>${escapeHtml(onlineNames.length ? onlineNames.join(', ') : 'Zatím nejsou data nebo domácnost není načtená.')}</em></div></div>
          <div class="setup-item ${localNames.length ? '' : 'done'}"><span>${localNames.length ? '!' : '✓'}</span><div><strong>Lokální-only / čeká na autosync</strong><em>${escapeHtml(localNames.length ? localNames.join(', ') : 'Nic aktuálně nečeká jen lokálně.')}</em></div></div>
          ${audit.riskNotes.map((item) => `<div class="setup-item"><span>•</span><div><strong>${escapeHtml(item.label)}</strong><em>${escapeHtml(item.note)}</em></div></div>`).join('')}
        </div>
      </section>
    `;
  }
  ...
  function getCalendarSource(sourceId) {
    return getCalendarModule().getCalendarSource(sourceId);
  }
  ...
  function renderCalendar() {
    return getCalendarModule().renderCalendar();
  }
  ...
  function getShoppingToolkit() {
    if (shoppingToolkitInstance) return shoppingToolkitInstance;
    const factory = window.DomacnostShoppingUtils?.createToolkit;
    if (typeof factory !== 'function') throw new Error('Shopping utils nejsou načtené');
    shoppingToolkitInstance = factory({
      defaultUnits: SHOPPING_UNITS,
      defaultCategories: SHOPPING_CATEGORIES,
      defaultCatalog: DEFAULT_SHOPPING_CATALOG,
      getState: () => state,
      normalizeText,
      normalizeKey,
      escapeHtml
    });
    return shoppingToolkitInstance;
  }

  function getShoppingCategoryIconMap() {
    return getShoppingToolkit().getShoppingCategoryIconMap();
  }

  function shoppingKindIcon(kind) {
    return getShoppingToolkit().shoppingKindIcon(kind);
  }

  function shoppingKindLabel(item) {
    return getShoppingToolkit().shoppingKindLabel(item);
  }
  ...
  function normalizeShoppingListRecord(list, index = 0) {
    return {
      ...
      cloudId: list?.cloudId || list?.cloud_id || list?.cloudListId || '',
      cloudListId: list?.cloudListId || list?.cloudId || list?.cloud_id || '',
      status: list?.status || 'active',
      createdAt: list?.createdAt || list?.created_at || new Date().toISOString(),
      updatedAt: list?.updatedAt || list?.updated_at || list?.createdAt || list?.created_at || new Date().toISOString(),
      sortOrder: Number(list?.sortOrder ?? list?.sort_order ?? index),
      source: list?.source || ''
    };
  }
  ...
  function markShoppingRuntimeDirty() {
    shoppingRuntimeReady = false;
  ...
  function ensureShoppingListsReady(force = false) {
    ...
    state.shopping = (state.shopping || []).map((item) => normalizeShoppingItemRecord(item, item.listId || state.activeShoppingListId || state.shoppingLists[0]?.id || ''));
    shoppingRuntimeReady = true;
  }
  ...
  function shoppingItemsForList(listId = getActiveShoppingListId()) {
    if (!listId) return [];
    return (state.shopping || []).filter((item) => item.listId === listId);
  }
  ...
  function buildShoppingListStats(lists = getShoppingLists()) {
    return getShoppingToolkit().buildShoppingListStats(lists);
  }

  function groupShoppingItemsByKind(items) {
    return getShoppingToolkit().groupShoppingItemsByKind(items);
  }

  function getShoppingUnits() {
    return getShoppingToolkit().getShoppingUnits();
  }

  function getShoppingCategories() {
    return getShoppingToolkit().getShoppingCategories();
  }

  function getShoppingCatalog() {
    return getShoppingToolkit().getShoppingCatalog();
  }

  function findShoppingCatalogItem(name) {
    return getShoppingToolkit().findShoppingCatalogItem(name);
  }

  function getShoppingStat(name) {
    return getShoppingToolkit().getShoppingStat(name);
  }
  ...
  function getShoppingQuickItems(limit = 10, catalog = getShoppingCatalog()) {
    return getShoppingToolkit().getShoppingQuickItems(limit, catalog);
  }

  function getShoppingDatalistItems(catalog = getShoppingCatalog(), limit = 90) {
    return getShoppingToolkit().getShoppingDatalistItems(catalog, limit);
  }

  function shoppingSourceLabel(item) {
    return getShoppingToolkit().shoppingSourceLabel(item);
  }

  function renderShoppingCatalogItem(item, activeList) {
    return getShoppingToolkit().renderShoppingCatalogItem(item, activeList);
  }

  function getShoppingRenderer() {
    if (shoppingRendererInstance) return shoppingRendererInstance;
    const factory = window.DomacnostShoppingRender?.createRenderer;
    if (typeof factory !== 'function') throw new Error('Shopping render není načtený');
    shoppingRendererInstance = factory({
      getState: () => state,
    ...
    return shoppingRendererInstance;
  }

  function renderShopping() {
    return getShoppingRenderer().renderShopping();
  }
  ...
  function getShoppingActions() {
    if (shoppingActionsInstance) return shoppingActionsInstance;
    const factory = window.DomacnostShoppingActions?.createController;
    if (typeof factory !== 'function') throw new Error('Shopping actions nejsou načtené');
    shoppingActionsInstance = factory({
      getState: () => state,
      normalizeText,
      normalizeKey,
      decimalValue,
    ...
    return shoppingActionsInstance;
  }

  function getWeatherModule() {
    if (weatherInstance) return weatherInstance;
    const factory = window.DomacnostWeather?.createWeather;
    if (!factory) throw new Error('weather.js není načtený');
    weatherInstance = factory({
      ...
      WEATHER_CHMI_FUNCTION
    });
    return weatherInstance;
  }

  function getPwaModule() {
    if (pwaInstance) return pwaInstance;
    const factory = window.DomacnostPwa?.createPwa;
    if (!factory) throw new Error('pwa.js není načtený');
    pwaInstance = factory({
      getState: () => state,
      saveState,
      render,
  ...
  function getNotesModule() {
    if (notesInstance) return notesInstance;
    const factory = window.DomacnostNotes?.createNotes;
  ...
  function getWarrantyModule() {
    ...
    warrantyInstance = factory({
      ...
      cloudAddExtraItem,
      cloudUpdateExtraItem,
      cloudDeleteExtraItem,
      WARRANTY_STATUS_OPTIONS,
      WARRANTY_YEARS_OPTIONS,
      WARRANTY_FILE_ALLOWED_MIME,
  ...
  function getFinanceModule() {
    if (financeInstance) return financeInstance;
    const factory = window.DomacnostFinance?.createFinance;
    if (!factory) throw new Error('finance.js není načtený');
    financeInstance = factory({
      getState: () => state,
      getFinanceEditId: () => financeEditId,
      setFinanceEditId: (value) => { financeEditId = value; },
      getFinanceAccountEditId: () => financeAccountEditId,
      setFinanceAccountEditId: (value) => { financeAccountEditId = value; },
      getFinanceTemplateEditId: () => financeTemplateEditId,
  ...
  function getContractsModule() {
    ...
    const factory = window.DomacnostContracts?.createContracts;
    if (!factory) throw new Error('contracts.js není načtený');
    contractsInstance = factory({
      getState: () => state,
  ...
  function getCalendarModule() {
    ...
    calendarInstance = factory({
      ...
      dateOffsetISO,
      localISODate,
      czechPublicHolidayName,
      pragueOffsetForLocalDateTime,
      getSupabaseClient,
      refreshCloudSession,
      cloudReady,
  ...
  function normalizeLoyaltyCard(item = {}) {
    ...
    return {
      ...
      note: normalizeText(item.note || ''),
      photoDataUrl: normalizeText(item.photoDataUrl || item.photo_data_url || item.codeImage || item.image || ''),
  ...
  function normalizeLoyaltyDetectedFormat(format = '') {
    const value = normalizeKey(format);
    if (value.includes('qr')) return 'qr';
    if (value.includes('code') || value.includes('ean') || value.includes('upc') || value.includes('aztec') || value.includes('data') || value.includes('pdf')) return 'barcode';
    return 'barcode';
  }

  function getLoyaltyScanState() {
    return loyaltyCardScan || { loading: false, dataUrl: '', detectedCode: '', detectedFormat: '', detectedText: '', error: '', source: '' };
  }

  function resetLoyaltyCardScan() {
    loyaltyCardScan = { loading: false, dataUrl: '', detectedCode: '', detectedFormat: '', detectedText: '', error: '', source: '' };
  }
  ...
  function renderLoyaltyScanPanel(scan = getLoyaltyScanState()) {
    ...
    const detectedText = normalizeText(scan?.detectedText || '');
    const sourceLabel = scan?.source === 'text' ? 'číslo z textu na fotce' : scan?.source === 'barcode' ? 'čárový/QR kód' : '';
    const status = scan?.loading
      ? 'Načítám fotku a zkouším najít kód i viditelné číslo…'
      : detected
        ? `Načteno ${sourceLabel ? `(${sourceLabel})` : ''}: ${detected}`
        : hasPreview
          ? 'Fotka je uložená jako podklad. Když se číslo nepřečetlo automaticky, opiš ho z náhledu do pole Číslo / kód karty.'
          : 'Vyfoť čárový/QR kód nebo kartu s viditelným číslem. Když to prohlížeč dovolí, číslo se doplní automaticky.';
    const statusClass = scan?.loading ? 'is-loading' : detected ? 'is-good' : hasPreview ? 'is-warn' : '';
    const textHint = detectedText && !detected ? `<span class="loyalty-detected-text">Na fotce jsem našel text: ${escapeHtml(detectedText)}</span>` : '';
    return `
      <div class="loyalty-scan-card ${statusClass}">
        <div class="loyalty-scan-copy">
          <strong>Vyfotit kartu / kód</strong>
  ...
  function readFileAsDataUrl(file) {
    return new Promise((resolve, reject) => {
      ...
      reader.onerror = () => reject(reader.error || new Error('Soubor nejde načíst'));
  ...
  async function compressLoyaltyImage(file) {
  ...
  function extractLoyaltyNumberFromText(text = '') {
    const source = normalizeText(text)
      .replace(/[O]/g, '0')
      .replace(/[Il|]/g, '1')
      .replace(/[–—]/g, '-');
    const numericCandidates = (source.match(/(?:\d[\s.\-]*){6,}/g) || [])
      .map((value) => value.replace(/[^0-9]/g, ''))
      .filter((value) => value.length >= 6);
    if (numericCandidates.length) return numericCandidates.sort((a, b) => b.length - a.length)[0];
    const alphaCandidates = (source.match(/[A-Z0-9][A-Z0-9\s.\-]{5,}/gi) || [])
      .map((value) => value.replace(/[^A-Z0-9]/gi, '').toUpperCase())
      .filter((value) => value.length >= 6);
    return alphaCandidates.sort((a, b) => b.length - a.length)[0] || '';
  }

  async function detectLoyaltyBarcodeFromImage(dataUrl) {
    const Detector = window.BarcodeDetector;
    if (typeof Detector !== 'function') return { code: '', format: '', error: 'BarcodeDetector není dostupný.' };
    try {
      const supported = typeof Detector.getSupportedFormats === 'function' ? await Detector.getSupportedFormats() : [];
      const detector = Array.isArray(supported) && supported.length ? new Detector({ formats: supported }) : new Detector();
      const image = await loadImageElement(dataUrl);
      const found = await detector.detect(image);
      const first = Array.isArray(found) ? found[0] : null;
      if (!first?.rawValue) return { code: '', format: '', error: 'Čárový/QR kód se z fotky nepovedlo načíst.' };
      return { code: normalizeText(first.rawValue), format: normalizeText(first.format || ''), source: 'barcode' };
    } catch (error) {
      console.warn('Loyalty barcode scan failed', error);
      return { code: '', format: '', error: 'Automatické čtení čárového/QR kódu se nepovedlo.' };
    }
  }

  async function detectLoyaltyTextFromImage(dataUrl) {
    const Detector = window.TextDetector;
    if (typeof Detector !== 'function') return { code: '', text: '', format: 'text', error: 'TextDetector není dostupný.' };
    try {
      const detector = new Detector();
      const image = await loadImageElement(dataUrl);
      const found = await detector.detect(image);
      const text = (Array.isArray(found) ? found : [])
        .map((item) => normalizeText(item.rawValue || item.text || ''))
        .filter(Boolean)
        .join(' ');
      const code = extractLoyaltyNumberFromText(text);
      if (!code) return { code: '', text, format: 'text', error: 'Text na fotce jsem našel, ale číslo karty z něj nešlo spolehlivě určit.' };
      return { code, text, format: 'text', source: 'text' };
    } catch (error) {
      console.warn('Loyalty text scan failed', error);
      return { code: '', text: '', format: 'text', error: 'Automatické čtení čísla z fotky se nepovedlo.' };
    }
  }

  async function detectLoyaltyCodeFromImage(dataUrl) {
    const barcode = await detectLoyaltyBarcodeFromImage(dataUrl);
    if (barcode.code) return barcode;
    const text = await detectLoyaltyTextFromImage(dataUrl);
    if (text.code) return text;
    return {
      code: '',
      format: '',
      detectedText: text.text || '',
      error: 'Fotku jsem uložil jako podklad, ale automatické čtení čísla tenhle prohlížeč nezvládl. Číslo opiš z náhledu ručně.'
    };
  }

  async function handleLoyaltyPhotoInput(input) {
    const file = input?.files?.[0];
    if (!file) return;
    loyaltyAddDetailsOpen = true;
    loyaltyCardScan = { loading: true, dataUrl: '', detectedCode: '', detectedFormat: '', detectedText: '', error: '', source: '' };
    render();
    try {
      ...
      loyaltyCardScan = {
        ...
        detectedText: detected.detectedText || detected.text || '',
        error: detected.code ? '' : detected.error || 'Fotka uložená. Číslo doplň ručně z náhledu.',
      ...
      if (loyaltyCardScan.detectedCode) {
        ...
        showToast(loyaltyCardScan.source === 'text' ? 'Číslo karty načtené z textu na fotce' : 'Kód karty načtený z fotky');
      } else {
        showToast('Fotka uložená jako podklad. Číslo opiš z náhledu do pole níž.');
      }
    } catch (error) {
      console.error('Loyalty photo load failed', error);
      loyaltyCardScan = { loading: false, dataUrl: '', detectedCode: '', detectedFormat: '', detectedText: '', error: 'Fotku se nepovedlo načíst.', source: '' };
    } finally {
      input.value = '';
    }
    render();
  ...
  async function handleLoyaltyEditPhotoInput(input) {
    ...
    try {
      ...
      render();
      showToast(detected.code ? (cloudReady() ? 'Kód karty načtený a uložený do domácnosti' : 'Kód karty načtený lokálně') : 'Fotka uložená. Číslo můžeš upravit ručně.');
    } catch (error) {
  ...
  function renderWarrantiesPanel(warranties) {
    return getWarrantyModule().renderWarrantiesPanel(warranties);
  }
  ...
  function notebookPageSummary(page) {
    return getNotesModule().notebookPageSummary(page);
  }

  function addNotebookPageFromForm(data, form) {
    return getNotesModule().addNotebookPageFromForm(data, form);
  }

  function addNotebookItemFromForm(data, form) {
    return getNotesModule().addNotebookItemFromForm(data, form);
  }

  function toggleNotebookItem(pageId, itemId) {
    return getNotesModule().toggleNotebookItem(pageId, itemId);
  }

  function deleteNotebookItem(pageId, itemId) {
    return getNotesModule().deleteNotebookItem(pageId, itemId);
  }

  function notebookItemToTask(pageId, itemId) {
    return getNotesModule().notebookItemToTask(pageId, itemId);
  }
  ...
  const READING_DETAIL_PERIOD_OPTIONS = [
    ['3', '3 měsíce'],
    ['6', '6 měsíců'],
    ['12', '12 měsíců'],
    ['24', '24 měsíců'],
  ...
  function normalizeReadingDeposits(item = {}) {
    return {
      electricity: normalizeReadingPricePart(item.electricity),
      gas: normalizeReadingPricePart(item.gas),
      water: normalizeReadingPricePart(item.water),
      updatedAt: normalizeText(item.updatedAt)
    };
  ...
  function readingMeterParent(meter = null) {
    const parentId = normalizeText(meter?.parentMeterId);
    if (!parentId || parentId === meter?.id) return null;
    return readingsMeters(true).find((item) => item.id === parentId) || null;
  }

  function readingChildMeters(meterId = '') {
    const cleanId = normalizeText(meterId);
    if (!cleanId) return [];
    return readingsMeters(true).filter((meter) => normalizeText(meter.parentMeterId) === cleanId);
  }

  function readingMeterDescendantIds(meterId = '', visited = new Set()) {
    const cleanId = normalizeText(meterId);
    if (!cleanId || visited.has(cleanId)) return visited;
    visited.add(cleanId);
    readingsMeters(true)
      .filter((meter) => normalizeText(meter.parentMeterId) === cleanId)
      .forEach((meter) => readingMeterDescendantIds(meter.id, visited));
    return visited;
  }

  function readingParentMeterOptions(type = '', unit = '', currentId = '') {
    const cleanCurrent = normalizeText(currentId);
    const blocked = readingMeterDescendantIds(cleanCurrent);
    const cleanType = normalizeText(type);
    const cleanUnit = normalizeText(unit);
    const options = readingsMeters(true)
      .filter((meter) => {
        if (!meter.id || meter.id === cleanCurrent || blocked.has(meter.id)) return false;
        if (cleanType && meter.type !== cleanType) return false;
        if (cleanUnit && meter.unit !== cleanUnit) return false;
        return !meter.archived;
      })
      .map((meter) => [meter.id, `${meter.name} · ${readingMeterGroup(meter)?.name || 'Domácnost'}`]);
    return [['', 'Samostatné / hlavní měřidlo'], ...options];
  }

  function readingMeterRelationLabel(meter = null) {
    const parent = readingMeterParent(meter);
    if (parent) return `podružné k ${parent.name}`;
    const children = readingChildMeters(meter?.id || '');
    if (children.length) return `hlavní · odečítá ${children.length} podružné`;
    return 'samostatné';
  }
  ...
  function readingMeterMonthlyDeposit(meter = null) {
    const direct = decimalValue(meter?.monthlyDeposit);
    if (direct !== '' && direct >= 0) return direct;
    return readingDepositForType(meter?.type || '', meter?.groupId || DEFAULT_READING_GROUP_ID);
  }

  function readingMeterMonthlyDepositSource(meter = null) {
    const direct = decimalValue(meter?.monthlyDeposit);
    if (direct !== '' && direct >= 0) return 'měřidlo';
    const fallback = readingDepositForType(meter?.type || '', meter?.groupId || DEFAULT_READING_GROUP_ID);
    return fallback !== '' ? 'místo' : '';
  }
  ...
  function normalizeReadingMeter(item = {}) {
    ...
    return {
      ...
      pricingMode,
      monthlyDeposit: normalizeReadingPricePart(item.monthlyDeposit ?? item.monthly_deposit ?? item.deposit ?? item.advancePayment ?? item.advance_payment),
      ...
      registers: Array.isArray(item.registers) ? item.registers.map((reg) => normalizeText(reg)).filter(Boolean) : [],
      note: normalizeText(item.note),
  ...
  function normalizeReadingEntry(item = {}) {
    ...
    return {
      ...
      register: normalizeText(item.register || item.importRegister),
  ...
  function readingsMeters(includeArchived = false) {
    const source = Array.isArray(state.readingMeters) ? state.readingMeters : [];
    if (source !== readingsMetersCacheSource) {
      readingsMetersCache = source.map(normalizeReadingMeter);
      const ids = new Set(readingsMetersCache.map((item) => item.id));
      readingsMetersCache = readingsMetersCache.map((item) => {
        if (!item.parentMeterId || item.parentMeterId === item.id || !ids.has(item.parentMeterId)) return { ...item, parentMeterId: '' };
        const parent = readingsMetersCache.find((candidate) => candidate.id === item.parentMeterId);
        if (!parent || parent.type !== item.type || parent.unit !== item.unit) return { ...item, parentMeterId: '' };
        return item;
      });
      readingsMetersCache = readingsMetersCache.map((item) => {
        const visited = new Set([item.id]);
        let parentId = item.parentMeterId;
        while (parentId) {
          if (visited.has(parentId)) return { ...item, parentMeterId: '' };
          visited.add(parentId);
          parentId = readingsMetersCache.find((candidate) => candidate.id === parentId)?.parentMeterId || '';
        }
        return item;
      });
      state.readingMeters = readingsMetersCache;
      readingsMetersCacheSource = state.readingMeters;
    }
    const list = includeArchived ? readingsMetersCache : readingsMetersCache.filter((item) => !item.archived);
  ...
  function readingsEntries(meterId = '') {
    const source = Array.isArray(state.readings) ? state.readings : [];
    if (source !== readingsEntriesCacheSource) {
      readingsEntriesCache = source
        .map(normalizeReadingEntry)
        .filter((item) => item.meterId && Number.isFinite(Number(item.value)))
  ...
  function readingEntryRegisterLabel(entry = {}) {
    const raw = normalizeText(entry.register || entry.importRegister);
    if (!raw) return '';
    if (raw === '1') return 'T1';
    if (raw === '2') return 'T2';
  ...
  function readingRegistersForMeter(meter = null) {
    const explicit = Array.isArray(meter?.registers) ? meter.registers.map((item) => normalizeText(item)).filter(Boolean) : [];
    const fromEntries = readingsEntries(meter?.id || '').map((entry) => readingEntryRegisterLabel(entry)).filter(Boolean);
    return Array.from(new Set([...explicit, ...fromEntries]));
  }

  function readingEntriesForRegister(meterId, registerLabel = '') {
  ...
  function previousReadingForEntry(entry) {
    const registerLabel = readingEntryRegisterLabel(entry);
    const rows = registerLabel
      ? readingEntriesForRegister(entry.meterId, registerLabel)
      : readingsEntries(entry.meterId).filter((item) => !readingEntryRegisterLabel(item)).sort((a, b) => String(a.date || '').localeCompare(String(b.date || '')) || String(a.createdAt || '').localeCompare(String(b.createdAt || '')));
  ...
  function readingMeterLatestDisplay(meter) {
    const registers = readingRegistersForMeter(meter);
    if (registers.length) {
      const parts = registers.slice(0, 3).map((register) => {
        const latest = [...readingEntriesForRegister(meter.id, register)].sort((a, b) => String(b.date || '').localeCompare(String(a.date || '')) || String(b.createdAt || '').localeCompare(String(a.createdAt || '')))[0];
        return latest ? `${register} ${readingValue(latest.value, meter.unit)}` : `${register} —`;
      });
      return parts.join(' · ');
    }
    const latest = latestReadingForMeter(meter.id);
    return latest ? readingValue(latest.value, meter.unit) : '—';
  }
    ...
    const contractFields = type === 'water'
      ...
      : type === 'gas'
        ...
        : `
          ...
          ${field('Stálý plat dodavateli', 'fixedSupplierMonthly', 'text', 'Kč/měsíc', false, meter.fixedSupplierMonthly ?? '', 'decimal')}
          ${field('Jiná stálá platba', 'otherFixedMonthly', 'text', 'Kč/měsíc', false, meter.otherFixedMonthly ?? '', 'decimal')}`;
    const contractModeFields = `${contractFields}${field('Popis stálých plateb', 'fixedMonthlyLabel', 'text', 'např. jistič + stálý plat', false, meter.fixedMonthlyLabel ?? '')}`;
    return `
      <details class="subtle-details readings-price-details" data-details-key="readings-price-${meter.id}" ${isDetailsOpen(`readings-price-${meter.id}`) ? 'open' : ''}>
        ...
        <div class="small-muted">V režimu průměru zadáš jen jednu cenu za jednotku. Přesný výpočet otevře položky ze smlouvy.</div>
        <div class="form-grid two">
          ${selectField('Výpočet ceny', 'pricingMode', readingPriceModeOptions, selectedMode)}
        </div>
        <fieldset class="form-grid two readings-price-mode-fields" data-reading-price-mode="average" ${selectedMode === 'average' ? '' : 'hidden disabled'}>
          ${directAverageFields}
        </fieldset>
        <fieldset class="form-grid two readings-price-mode-fields" data-reading-price-mode="contract" ${selectedMode === 'contract' ? '' : 'hidden disabled'}>
          ${contractModeFields}
        </fieldset>
  ...
  function syncReadingPriceModeFields(form) {
    ...
    form.querySelectorAll('[data-reading-price-mode]').forEach((section) => {
      const active = section.dataset.readingPriceMode === mode;
  ...
  function readingMeterStats(meter, precomputedRows = null) {
    ...
    const delta = lastConsumption ? lastConsumption.value : null;
    return { rows, latest, previous: null, delta };
  }
  ...
  function readingsConsumptionRows() {
    ...
    readingsMeters().forEach((meter) => {
      ...
      groups.forEach((registerLabel) => {
        ...
        sorted.forEach((entry, index) => {
          ...
          const fixedMonthlyCost = readingFixedMonthlyCost(meter);
          const hasUnitPrice = unitPrice !== '';
          const variableMonthlyCost = monthlyUsage > 0 && hasUnitPrice ? monthlyUsage * unitPrice : 0;
          ...
          const current = map.get(key) || {
            ...
            isSubmeter: Boolean(meter.parentMeterId),
          ...
          current.rawValue += monthlyUsage;
          current.rawTotalValue += totalUsage;
          ...
          if (registerLabel) current.registerValues.push({ register: registerLabel, value: monthlyUsage, unitPrice: hasUnitPrice ? unitPrice : '' });
          if (unitPrice !== '') current.unitPrice = unitPrice;
          if (hasUnitPrice) current.hasCost = true;
    ...
    const rows = [...map.values()]
      .map((row) => ({
        ...
        rawValue: Number((row.rawValue || row.value).toFixed(3)),
        rawTotalValue: Number((row.rawTotalValue || row.totalValue || row.value).toFixed(3)),
        submeterValue: Number((row.submeterValue || 0).toFixed(3)),
        ...
        registers: Array.isArray(row.registers) ? row.registers : [],
    ...
    const rowByMeterMonth = new Map(rows.map((row) => [`${row.meterId}|${row.month}`, row]));
    rows.forEach((row) => {
      if (!row.parentMeterId) return;
      const parent = rowByMeterMonth.get(`${row.parentMeterId}|${row.month}`);
      if (!parent || parent.type !== row.type || parent.unit !== row.unit) return;
      const childValue = Math.max(0, Number(row.value) || 0);
      const childTotalValue = Math.max(0, Number(row.totalValue) || 0);
      parent.submeterValue = Number(((Number(parent.submeterValue) || 0) + childValue).toFixed(3));
      parent.submeterTotalValue = Number(((Number(parent.submeterTotalValue) || 0) + childTotalValue).toFixed(3));
      parent.submeterNames = Array.from(new Set([...(parent.submeterNames || []), row.meterName].filter(Boolean)));
      parent.value = Number(Math.max(0, (Number(parent.rawValue) || 0) - parent.submeterValue).toFixed(3));
      parent.totalValue = Number(Math.max(0, (Number(parent.rawTotalValue || parent.totalValue) || 0) - (Number(parent.submeterTotalValue) || 0)).toFixed(3));
      const parentUnitPrice = parent.unitPrice !== '' ? Number(parent.unitPrice) : '';
      if (parentUnitPrice !== '' && Number.isFinite(parentUnitPrice)) {
        const fixed = Number(parent.fixedMonthlyCost || 0);
        parent.cost = Number(Math.max(fixed, (Number(parent.cost) || 0) - (childValue * parentUnitPrice)).toFixed(2));
        parent.hasCost = parent.cost > 0 || Boolean(parent.hasCost);
      }
    });
    rows.forEach((row) => {
      row.netValue = row.value;
      row.hasSubmeters = (Number(row.submeterValue) || 0) > 0;
    });
    rows.sort((a, b) => String(b.month).localeCompare(String(a.month)) || String(a.meterName).localeCompare(String(b.meterName), 'cs'));
    readingsConsumptionCacheMetersSource = metersSource;
    readingsConsumptionCacheEntriesSource = entriesSource;
    readingsConsumptionCachePricesSource = pricesSource;
    readingsConsumptionCacheGroupsSource = groupsSource;
  ...
  function readingsMonthlyRows(sourceRows = null) {
    ...
    rows.forEach((row) => {
      ...
      if (row.hasCost) current.hasCost = true;
  ...
  function readingCostValueLabel(row = {}) {
    if (!row || !row.hasCost) return 'bez ceny';
    return `${readingMoney(row.cost)}/měs.`;
  }

  function readingCostBalance(meter = null, row = null) {
    const deposit = readingMeterMonthlyDeposit(meter);
    const depositSource = readingMeterMonthlyDepositSource(meter);
    const estimate = row?.hasCost ? Number(row.cost || 0) : null;
    const diff = estimate !== null && deposit !== '' ? Number((deposit - estimate).toFixed(2)) : null;
    if (estimate === null) return { deposit, depositSource, estimate, diff, status: deposit !== '' ? 'waiting' : 'missing', label: deposit !== '' ? 'čeká na spotřebu' : 'bez porovnání' };
    if (deposit === '') return { deposit, depositSource, estimate, diff, status: 'missing', label: 'záloha nenastavená' };
    const tolerance = Math.max(100, estimate * 0.1);
    const highReserve = Math.max(250, estimate * 0.25);
    const status = diff < -tolerance ? 'low' : diff > highReserve ? 'high' : 'ok';
    const label = status === 'low'
      ? `nízká o ${readingMoney(Math.abs(diff))}/měs.`
      : status === 'high'
        ? `rezerva ${readingMoney(diff)}/měs.`
        : `v pořádku ${diff >= 0 ? '+' : ''}${readingMoney(diff)}/měs.`;
    return { deposit, depositSource, estimate, diff, status, label };
  }

  function readingCostBalanceClass(status = '') {
  ...
  function readingCostSummaryRows(monthRows = []) {
    ...
    monthRows.forEach((row) => {
      if (row.meterId && !latestByMeter.has(row.meterId)) latestByMeter.set(row.meterId, row);
    });
    return readingsMeters().map((meter) => {
      const row = latestByMeter.get(meter.id) || null;
      const balance = readingCostBalance(meter, row);
      const group = readingMeterGroup(meter);
      return { meter, group, row, ...balance };
    });
  }
    ...
    return `<div class="readings-cost-summary">
      ${rows.map((item) => {
        ...
        return `<article class="mini-stat readings-cost-card">
          ...
          <small>platíš: ${hasDeposit ? `${readingMoney(item.deposit)}/měs.${depositSource}` : 'záloha nenastavená'} · <b class="${diffClass}">${escapeHtml(item.label)}</b></small>
  ...
  function readingLastMonthDisplay(row = null, meter = null) {
    if (!row) return '';
    const registers = Array.isArray(row.registerValues) ? row.registerValues : [];
    const registerText = registers.length
      ? ` · ${registers.map((item) => `${item.register} ${readingMonthlyValue(item.value, meter?.unit || row.unit || '')}`).join(' + ')}`
      : '';
    const costText = row.hasCost ? ` · ${readingMoney(row.cost)}/měs.` : ' · bez ceny';
    const submeterText = row.hasSubmeters ? ` · čistě po odečtení ${readingMonthlyValue(row.submeterValue, meter?.unit || row.unit || '')}` : '';
    return `měsíční průměr ${readingMonthlyValue(row.value, meter?.unit || row.unit || '')}${registerText}${submeterText}${costText}`;
  }
      ...
      <article class="readings-group-card">
        <div class="readings-group-head">
          <div>
            ...
            <span>${id === DEFAULT_READING_GROUP_ID ? 'výchozí místo – jde přejmenovat třeba na Dům' : 'samostatné místo / domácnost'}</span>
          </div>
          ${canDelete ? `<button class="danger-btn small-btn" type="button" data-action="delete-reading-group" data-id="${escapeHtml(id)}">Smazat místo</button>` : ''}
        </div>
        <div class="form-grid two">
          ${field('Název místa', `groupName${suffix}`, 'text', 'např. Dům / Byt k pronájmu / Garáž', true, clean.name)}
  ...
  function renderReadingGlobalPricesPanel() {
    ...
    return `
      <section class="soft-panel readings-tool-page readings-prices-page">
        <div class="card-subheader"><div><h3>Místa, ceny a zálohy</h3><p>Rozděl měřidla podle toho, kam patří: Dům, byt k pronájmu, dílna nebo garáž. Každé místo může mít vlastní ceny, zálohy a fakturační období.</p></div></div>
        <form data-form="add-reading-group" class="compact-form readings-form readings-add-group-form">
          <div class="form-grid two">
            ${field('Nové místo / domácnost', 'name', 'text', 'např. Byt k pronájmu / Dílna', true)}
            <div class="form-actions align-end"><button class="primary-btn" type="submit">Přidat místo</button></div>
          </div>
        </form>
        <form data-form="update-reading-prices" class="compact-form readings-form">
          <div class="mini-section-title">Místa / domácnosti</div>
          <div class="readings-groups-settings">
            ${groups.map(renderReadingGroupSettingsCard).join('')}
          </div>
          <p class="small-muted">Fakturační období je zvlášť pro elektřinu, plyn a vodu, protože každé vyúčtování může chodit v jiném termínu. Když měřidlo nemá vlastní cenu, použije se cena jeho místa.</p>
          <div class="form-actions"><button class="primary-btn" type="submit">Uložit místa</button></div>
        </form>
      </section>`;
  }
  ...
  async function updateReadingPricesFromForm(data, form) {
    ...
    form?.reset?.();
    await persistReadingsState('Místa uložená');
  }

  async function addReadingGroupFromForm(data, form) {
    const name = normalizeText(data.name);
    if (!name) return showToast('Doplň název místa');
    ...
    readingsMeterToolPage = 'prices';
      localStorage.setItem('domacnostPlus.readingsMeterToolPage', readingsMeterToolPage);
    await persistReadingsState('Místo přidané');
  }
  ...
  async function deleteReadingGroup(id) {
    ...
    if (!group) return;
    if (groups.length <= 1 || cleanId === DEFAULT_READING_GROUP_ID) return showToast('Výchozí místo nejde smazat');
  ...
  async function addReadingEntryFromForm(data, form) {
    ...
    const photoFile = data.photo && typeof File !== 'undefined' && data.photo instanceof File && data.photo.name ? data.photo : null;
    const baseNote = normalizeText(data.note);
    const note = [baseNote, photoFile ? `Foto odečtu: ${photoFile.name}` : ''].filter(Boolean).join(' · ');
    const registerEntries = Object.keys(data)
      .filter((key) => key.startsWith('registerValue__'))
      .map((key) => {
        const suffix = key.replace('registerValue__', '');
        return { register: normalizeText(data[`registerLabel__${suffix}`] || suffix), value: decimalValue(data[key]) };
      })
      .filter((item) => item.register && Number.isFinite(item.value) && item.value >= 0);
    if (registerEntries.length) {
      const confirmOlder = registerEntries.some((item) => {
  ...
  function easyHomeImportRowsToState(rows = []) {
    ...
    const getMeter = (baseName, register, serial = '') => {
      ...
      const importKey = isElectricity ? `easyhome|${normalizeKey(baseName)}|electricity` : `easyhome|${normalizeKey(baseName)}|${register}`;
      const registerLabel = isElectricity ? (register === '2' ? 'T2' : 'T1') : '';
      const label = isElectricity ? baseName : (register === '2' ? `${baseName} · odečet 2` : baseName);
      ...
      if (!meter) {
        meter = normalizeReadingMeter({
          ...
          unit,
          serial,
          note: isElectricity ? 'Import z aplikace Domácí odečty · dvoutarif T1/T2' : 'Import z aplikace Domácí odečty',
          pricingMode: 'average',
          importSource: 'easy-home-offtake',
          importKey,
          importRegister: isElectricity ? 'T1/T2' : register,
          registers: registerLabel ? [registerLabel] : [],
          createdAt: now,
          updatedAt: now
        });
      ...
      } else if (registerLabel && !readingRegistersForMeter(meter).includes(registerLabel)) {
        meter.registers = Array.from(new Set([...(meter.registers || []), registerLabel]));
        meter.updatedAt = now;
      }
      return { meter, registerLabel };
    };
    dataRows.forEach((row) => {
      ...
      [
        ...
        const { meter, registerLabel } = getMeter(baseName, register, serial);
        const importKey = `easyhome|${normalizeKey(baseName)}|${registerLabel || register}|${date}|${value}`;
        const legacyImportKey = `easyhome|${normalizeKey(baseName)}|${register}|${date}|${value}`;
        const genericKey = `${meter.id}|${registerLabel || ''}|${date}|${value}`;
        const legacyGenericKey = `${meter.id}|${date}|${value}`;
        if (entryKeys.has(importKey) || entryKeys.has(legacyImportKey) || entryKeys.has(genericKey) || entryKeys.has(legacyGenericKey)) return;
        entryKeys.add(importKey);
        entryKeys.add(legacyImportKey);
        entryKeys.add(genericKey);
        entryKeys.add(legacyGenericKey);
        entryAdds.push(normalizeReadingEntry({
          meterId: meter.id,
          date,
          value,
          register: registerLabel,
          note: registerLabel ? [registerLabel, note].filter(Boolean).join(' · ') : note,
          importSource: 'easy-home-offtake',
          importKey,
          createdAt: now,
          updatedAt: now
  ...
  function readingCurrentMonthKey() {
    return todayISO().slice(0, 7);
  }

  function readingRegisterInputKey(register = '') {
    return normalizeText(register).toUpperCase().replace(/[^A-Z0-9]+/g, '_') || 'VALUE';
  }

  function readingMeterMonthEntries(meterId, monthKey = readingCurrentMonthKey()) {
    return readingsEntries(meterId).filter((entry) => String(entry.date || '').slice(0, 7) === monthKey);
  }

  function readingMeterHasCompleteMonthEntry(meter, monthKey = readingCurrentMonthKey()) {
    const monthEntries = readingMeterMonthEntries(meter.id, monthKey);
    if (!monthEntries.length) return false;
    const registers = readingRegistersForMeter(meter);
    if (!registers.length) return true;
    const present = new Set(monthEntries.map((entry) => readingEntryRegisterLabel(entry)).filter(Boolean));
    return registers.every((register) => present.has(normalizeText(register).toUpperCase()));
  }

  function readingsPeriodFilteredEntries(entries = [], period = readingsDetailPeriod) {
  ...
  function renderReadingsEntryPanel(meters = []) {
    ...
    if (!readingsEntryMeterId || !meters.some((meter) => meter.id === readingsEntryMeterId)) readingsEntryMeterId = dueMeters[0]?.id || meters[0].id;
    const selectedMeter = readingMeterById(readingsEntryMeterId) || dueMeters[0] || meters[0];
    return `
      <div class="card-subheader readings-entry-head">
        ...
        <details class="action-details compact-edit-details readings-form-drawer readings-entry-manual" ${readingsEntryDrawerOpen ? 'open' : ''}>
      ...
      ${dueMeters.length ? `<div class="readings-due-grid">${dueMeters.map((meter) => {
        const meta = readingTypeMeta(meter.type);
        const latest = latestReadingForMeter(meter.id);
        const selected = meter.id === selectedMeter.id && readingsEntryDrawerOpen;
  ...
  function renderReadingDetailPanel(meters) {
    ...
    if (!readingsDetailMeterId || !meters.some((meter) => meter.id === readingsDetailMeterId)) readingsDetailMeterId = meters[0].id;
  ...
  function renderReadings() {
    ...
    if (activeTab === 'overview') {
      ...
      const priceFormBlock = readingsMeterToolPage === 'add' ? renderReadingPriceFormBlock() : '';
      content = `
  ...
  function garageRowsForVehicle(vehicleId) {
    return {
      fuelRows: sortFuelRows((state.fuel || []).filter((item) => item.vehicleId === vehicleId)),
      serviceRows: (state.services || []).filter((item) => item.vehicleId === vehicleId).sort((a, b) => String(a.date || '').localeCompare(String(b.date || '')))
    };
  }
  ...
  function garageCurrentMonthKey() {
    return todayISO().slice(0, 7);
  }

  function garagePreviousYear() {
    return new Date().getFullYear() - 1;
  }

  function garageFuelEntryMetrics(fuelRows = []) {
    ...
    for (let index = 1; index < rows.length; index += 1) {
      ...
      const currentKm = Number(current.odometer || 0);
      const liters = Number(current.liters || 0);
      const price = Number(current.price || 0);
      const km = currentKm - previousKm;
      if (currentKm > previousKm && liters > 0) {
        entries.push({ ...current, km, liters, price, consumption: (liters / km) * 100, pricePerLiter: fuelPricePerLiter(current) });
      }
    }
    return entries;
  }
  ...
  function garageVehicleAnalytics(vehicle) {
    ...
    const entries = garageFuelEntryMetrics(fuelRows);
    const nowMonth = garageCurrentMonthKey();
    const prevYear = garagePreviousYear();
    const thisMonthFuel = fuelRows.filter((item) => garageMonthKey(item.date) === nowMonth);
    ...
    const purchaseKm = Number(vehicle?.purchaseOdometer || 0);
    const saleKm = Number(vehicle?.saleOdometer || 0);
    const firstKnownKm = purchaseKm || (odometerValues.length ? Math.min(...odometerValues) : Number(vehicle?.odometerStart || vehicle?.startOdometer || 0));
    const endKnownKm = saleKm > 0 ? saleKm : currentKm;
    const totalDistance = endKnownKm && firstKnownKm && endKnownKm > firstKnownKm ? endKnownKm - firstKnownKm : stats.totalKm || 0;
    const dates = [parseDateValue(vehicle?.purchaseDate), ...fuelRows.map((item) => parseDateValue(item.date)), parseDateValue(vehicle?.saleDate)].filter(Boolean).sort((a, b) => a - b);
    const ownershipMonths = vehicleOwnershipMonths(vehicle);
    const monthSpan = ownershipMonths || (dates.length >= 2 ? Math.max(1, ((dates[dates.length - 1].getFullYear() - dates[0].getFullYear()) * 12) + (dates[dates.length - 1].getMonth() - dates[0].getMonth()) + 1) : 1);
    const prevYearEntries = entries.filter((item) => Number(String(item.date || '').slice(0, 4)) === prevYear);
    const prevYearDistance = prevYearEntries.reduce((sum, item) => sum + Number(item.km || 0), 0);
    const bestConsumption = entries.length ? Math.min(...entries.map((item) => item.consumption).filter((value) => Number.isFinite(value) && value > 0)) : null;
    const worstConsumption = entries.length ? Math.max(...entries.map((item) => item.consumption).filter((value) => Number.isFinite(value) && value > 0)) : null;
    return {
      fuelRows,
      serviceRows,
      stats,
      ...
      fuelCountPrevYear: prevYearFuel.length,
      litersTotal: fuelRows.reduce((sum, item) => sum + Number(item.liters || 0), 0),
      litersThisMonth: thisMonthFuel.reduce((sum, item) => sum + Number(item.liters || 0), 0),
      litersPrevYear: prevYearFuel.reduce((sum, item) => sum + Number(item.liters || 0), 0),
      bestConsumption,
      worstConsumption,
      serviceCostTotal: serviceRows.reduce((sum, item) => sum + Number(item.price || 0), 0),
      serviceCountTotal: serviceRows.length,
      serviceCostThisMonth: serviceRows.filter((item) => garageMonthKey(item.date) === nowMonth).reduce((sum, item) => sum + Number(item.price || 0), 0),
      serviceCostPrevYear: serviceRows.filter((item) => Number(String(item.date || '').slice(0, 4)) === prevYear).reduce((sum, item) => sum + Number(item.price || 0), 0),
      totalDistance,
      averageMonthDistance: totalDistance / monthSpan,
      averageYearDistance: (totalDistance / monthSpan) * 12,
      prevYearDistance
    };
  }
  ...
  function garageEdgeLabels(points = []) {
    const rows = Array.isArray(points) ? points.filter((item) => item && Number(item.value || 0) > 0) : [];
  ...
  function garageChartPointsFromConsumption(entries = [], limit = 14) {
    return [...entries]
      .sort((a, b) => String(a.date || '').localeCompare(String(b.date || '')))
      .map((item) => ({ value: Number(item.consumption || 0), label: formatDate(item.date) }))
      .filter((item) => Number(item.value || 0) > 0)
      .slice(-limit);
  }

  function garageMonthlyFuelCostPoints(fuelRows = []) {
    const map = new Map();
    fuelRows.forEach((item) => {
      const key = garageMonthKey(item.date);
      if (!key) return;
      map.set(key, (map.get(key) || 0) + Number(item.price || 0));
    });
    return [...map.entries()].sort((a, b) => a[0].localeCompare(b[0])).map(([key, value]) => ({ value, label: financeMonthLabel(key) }));
  }
  ...
  function renderContracts() {
    return getContractsModule().renderContracts();
  }

  function contractFileCount(contractId) {
    return getContractsModule().contractFileCount(contractId);
  }

  function addContractFromForm(data, form) {
    return getContractsModule().addContractFromForm(data, form);
  }

  function updateContract(id, data, form) {
    return getContractsModule().updateContract(id, data, form);
  }

  function cloudAddContract(contract) {
    return getContractsModule().cloudAddContract(contract);
  }

  function cloudUpdateContract(contract) {
    return getContractsModule().cloudUpdateContract(contract);
  }

  function cloudLoadContracts(showMessage = true) {
  ...
  function cloudSyncLocalContracts() {
  ...
  function subscriptionSelectedMonth() {
    return getSubscriptionsModule().subscriptionSelectedMonth();
  }

  function subscriptionPaymentFilter() {
    return getSubscriptionsModule().subscriptionPaymentFilter();
  }

  function renderSubscriptions() {
    return getSubscriptionsModule().renderSubscriptions();
  }

  function normalizeSubscriptionService(item) {
    return getSubscriptionsModule().normalizeSubscriptionService(item);
  }

  function normalizeSubscriptionPerson(person) {
    return getSubscriptionsModule().normalizeSubscriptionPerson(person);
  }

  function normalizeSubscriptionPayment(item) {
    return getSubscriptionsModule().normalizeSubscriptionPayment(item);
  }

  function getSubscriptionPeople() {
    return getSubscriptionsModule().getSubscriptionPeople();
  }

  function getSubscriptionServices() {
    return getSubscriptionsModule().getSubscriptionServices();
  }

  function getSubscriptionPayments() {
    return getSubscriptionsModule().getSubscriptionPayments();
  }

  function setSubscriptionMonth(month) {
    return getSubscriptionsModule().setSubscriptionMonth(month);
  }
  ...
  function setSubscriptionPaymentDraft(partial) {
    return getSubscriptionsModule().setSubscriptionPaymentDraft(partial);
  }

  function addSubscriptionPersonFromForm(data, form) {
    return getSubscriptionsModule().addSubscriptionPersonFromForm(data, form);
  }
  ...
  function toggleSubscriptionService(id) {
    return getSubscriptionsModule().toggleSubscriptionService(id);
  }

  function deleteSubscription(id) {
    return getSubscriptionsModule().deleteSubscription(id);
  }

  function deleteSubscriptionPerson(id) {
    return getSubscriptionsModule().deleteSubscriptionPerson(id);
  }

  function deleteSubscriptionShare(subscriptionId, personId) {
    return getSubscriptionsModule().deleteSubscriptionShare(subscriptionId, personId);
  }

  function deleteSubscriptionPayment(id) {
    return getSubscriptionsModule().deleteSubscriptionPayment(id);
  }
    ...
    return `
      <div class="grid two more-hub-grid more-clean-hub">
        <section class="card desktop-span-2 more-settings-priority">
          <div class="card-header"><div><h2>Nastavení aplikace</h2><p>Domácnost, profily, zapnuté moduly, cloud, PWA a export/import.</p></div></div>
  ...
  function renderAuthSetupCard() {
    ...
    const templatePath = 'email-confirmation-template-domacnost-plus.html';
    const checks = [
      {
        ok: true,
  ...
  // PWA render/status/diagnostics — přesunuto do pwa.js.
  function renderPwaUpdateStatusCard() {
    return getPwaModule().renderPwaUpdateStatusCard();
  }
  function renderPwaInstallCard() {
    return getPwaModule().renderPwaInstallCard();
  }
  ...
  function cloudSyncLocalWarrantyFiles(showMessage = true) {
    return getWarrantyModule().cloudSyncLocalWarrantyFiles(showMessage);
  }

  function addWarrantyFiles(form) {
    return getWarrantyModule().addWarrantyFiles(form);
  }

  function openWarrantyFile(id) {
    return getWarrantyModule().openWarrantyFile(id);
  }

  function deleteWarrantyFile(id) {
    return getWarrantyModule().deleteWarrantyFile(id);
  }
  ...
  async function updateVehicle(vehicleId, data) {
    ...
    vehicle.ownershipStatus = normalizeVehicleOwnershipStatus(data.ownershipStatus || vehicle.ownershipStatus);
  ...
  async function quickAddShoppingByName(name) {
    return getShoppingActions().quickAddShoppingByName(name);
  }


  async function addShoppingListFromForm(data, form) {
    return getShoppingActions().addShoppingListFromForm(data, form);
  }


  function setActiveShoppingList(id) {
    return getShoppingActions().setActiveShoppingList(id);
  }


  function deleteShoppingList(id) {
    return getShoppingActions().deleteShoppingList(id);
  }


  function promptAddShoppingList() {
    return getShoppingActions().promptAddShoppingList();
  }


  function updateShoppingQuantity(id, delta) {
    return getShoppingActions().updateShoppingQuantity(id, delta);
  }


  function deleteDoneShoppingBySwipe(id) {
    return getShoppingActions().deleteDoneShoppingBySwipe(id);
  }
  ...
  async function cloudLoadShoppingData(showMessage = true) {
    ...
    const [unitsRes, categoriesRes, catalogRes, listsRes] = await Promise.all([
      ...
      client.from('shopping_lists').select('id,name,status,created_at,updated_at').eq('household_id', householdId).eq('status', 'active').order('created_at', { ascending: true })
    ]);
    ...
    const cloudListsRaw = listsRes.data || [];
    const existingByCloudId = new Map((state.shoppingLists || []).filter((list) => list.cloudId || list.cloudListId).map((list) => [list.cloudId || list.cloudListId, list]));
    const existingByName = new Map((state.shoppingLists || []).map((list) => [normalizeKey(list.name), list]));
    const cloudLists = cloudListsRaw.map((list, index) => {
      const existing = existingByCloudId.get(list.id) || existingByName.get(normalizeKey(list.name));
      return normalizeShoppingListRecord({
        id: existing?.id || `shopping-list-cloud-${list.id}`,
        cloudId: list.id,
        cloudListId: list.id,
        householdId: currentHouseholdId(),
        profileId: currentProfileId(),
        name: list.name || `Seznam ${index + 1}`,
        status: list.status || 'active',
        createdAt: list.created_at || new Date().toISOString(),
        updatedAt: list.updated_at || list.created_at || new Date().toISOString(),
        sortOrder: index,
        source: 'cloud'
      }, index);
    });
    const cloudNameKeys = new Set(cloudLists.map((list) => normalizeKey(list.name)));
    const localOnlyLists = (state.shoppingLists || []).filter((list) => !(list.cloudId || list.cloudListId) && !cloudNameKeys.has(normalizeKey(list.name)));
    state.shoppingLists = normalizeShoppingLists([...localOnlyLists, ...cloudLists], state);
    const cloudIdToLocalId = new Map(state.shoppingLists.filter((list) => list.cloudId || list.cloudListId).map((list) => [list.cloudId || list.cloudListId, list.id]));
    ...
    if (!cloudLists.length) {
      ...
      saveState();
      render();
      if (showMessage) showToast('V cloudu zatím není žádný nákupní seznam');
      return true;
    }
  ...
  async function cloudAddShoppingList(name) {
    const client = getSupabaseClient();
    ...
    const { data, error } = await client
      ...
      .single();
    if (error) {
      showToast(error.message || 'Seznam se nepovedlo vytvořit v cloudu');
      return null;
    }
    state.cloud.lastSyncAt = new Date().toISOString();
    return data;
  ...
  async function cloudArchiveShoppingList(list) {
    const client = getSupabaseClient();
    const cloudListId = list?.cloudId || list?.cloudListId || '';
  ...
  async function cloudAddShoppingItem({ name, category, quantity, unit, note, catalogItem, list = null }) {
    const client = getSupabaseClient();
    if (!client || !state.cloud?.householdId) return null;
  ...
  function markGarageVehicleExtendedSchemaPending() {
    garageVehicleExtendedSchemaPending = true;
    state.cloud = state.cloud && typeof state.cloud === 'object' ? state.cloud : {};
    state.cloud.garageVehicleExtendedColumnsPending = true;
  }
  ...
  function cloudLoadCalendarSources(showMessage) {
    return getCalendarModule().cloudLoadCalendarSources(showMessage);
  }

  function mergeCalendarSources(current, incoming) {
    return getCalendarModule().mergeCalendarSources(current, incoming);
  }

  function addCalendarSourceFromForm(data, form) {
    return getCalendarModule().addCalendarSourceFromForm(data, form);
  }

  function cloudSyncLocalCalendarSources(showMessage) {
  ...
  function toggleCalendarSource(sourceId, enabled) {
    return getCalendarModule().toggleCalendarSource(sourceId, enabled);
  }

  function deleteCalendarSource(sourceId) {
    return getCalendarModule().deleteCalendarSource(sourceId);
  }

  function cloudLoadCalendar(showMessage) {
    return getCalendarModule().cloudLoadCalendar(showMessage);
  }

  function cloudSyncCalendarById(id) {
    return getCalendarModule().cloudSyncCalendarById(id);
  }

  function cloudSyncLocalCalendar() {
    return getCalendarModule().cloudSyncLocalCalendar();
  }

  function addEventFromForm(data, form) {
    return getCalendarModule().addEventFromForm(data, form);
  }
  ...
  function addWarrantyFromForm(data, form) {
    return getWarrantyModule().addWarrantyFromForm(data, form);
  }

  function updateWarrantyFromForm(data, form) {
    return getWarrantyModule().updateWarrantyFromForm(data, form);
  }
  ...
  function cloudLoadTasks(showMessage = true) {
    return getNotesModule().cloudLoadTasks(showMessage);
  }

  function cloudSyncTaskById(id) {
    return getNotesModule().cloudSyncTaskById(id);
  }

  function cloudSyncLocalTasks() {
    return getNotesModule().cloudSyncLocalTasks();
  }

  function addTaskFromForm(data, form) {
    return getNotesModule().addTaskFromForm(data, form);
  }

  function toggleTaskDone(id) {
    return getNotesModule().toggleTaskDone(id);
  }

  function deleteTask(id) {
    return getNotesModule().deleteTask(id);
  }

  async function toggleShoppingDone(id) {
    return getShoppingActions().toggleShoppingDone(id);
  }


  async function deleteShoppingItem(id) {
    return getShoppingActions().deleteShoppingItem(id);
  }
    ...
    const handlers = {
      ...
      'add-vehicle': async () => {
        const vehicle = {
          ...
          ownershipStatus: normalizeVehicleOwnershipStatus(data.ownershipStatus),
      ...
      'garage-trip-calc': () => {
        ...
        const fuelPrice = decimalValue(data.fuelPrice);
        const liters = distance > 0 && consumption > 0 ? (distance * consumption) / 100 : 0;
      ...
      'add-contract': () => addContractFromForm(data, form),
  ...
  function financeTypeFilter() {
    return getFinanceModule().financeTypeFilter();
  }

  function setFinanceMonth(month) {
    return getFinanceModule().setFinanceMonth(month);
  }

  function shiftFinanceMonth(delta) {
    return getFinanceModule().shiftFinanceMonth(delta);
  }

  function setFinanceTypeFilter(filter) {
    return getFinanceModule().setFinanceTypeFilter(filter);
  }

  function addFinanceAccountFromForm(data, form) {
    return getFinanceModule().addFinanceAccountFromForm(data, form);
  }

  function updateFinanceAccountFromForm(id, data, form) {
    return getFinanceModule().updateFinanceAccountFromForm(id, data, form);
  }

  function addManagedFinanceSetFromForm(data, form) {
    return getFinanceModule().addManagedFinanceSetFromForm(data, form);
  }

  function addFinanceTemplateFromForm(data, form) {
    return getFinanceModule().addFinanceTemplateFromForm(data, form);
  }

  function updateFinanceTemplateFromForm(id, data, form) {
    return getFinanceModule().updateFinanceTemplateFromForm(id, data, form);
  }

  function addFinanceFromForm(data, form) {
    return getFinanceModule().addFinanceFromForm(data, form);
  }

  function updateFinanceFromForm(id, data, form) {
  ...
  function cloudSyncLocalFinanceAccounts() {
    return getFinanceModule().cloudSyncLocalFinanceAccounts();
  }

  function cloudSyncLocalFinance() {
    return getFinanceModule().cloudSyncLocalFinance();
  }

  function cloudLoadFinance(showMessage = true) {
    return getFinanceModule().cloudLoadFinance(showMessage);
  }

  function setFinanceAccountEdit(id) {
    return getFinanceModule().setFinanceAccountEdit(id);
  }

  function cloudSyncFinanceAccountById(id) {
    return getFinanceModule().cloudSyncFinanceAccountById(id);
  }

  function deleteFinanceAccount(id) {
    return getFinanceModule().deleteFinanceAccount(id);
  }

  function cloudSyncAllFinance() {
    return getFinanceModule().cloudSyncAllFinance();
  }

  function setFinanceEdit(id) {
    return getFinanceModule().setFinanceEdit(id);
  }

  function setFinanceCopy(id) {
    return getFinanceModule().setFinanceCopy(id);
  }

  function saveFinanceTemplateFromVisibleForm(form) {
    return getFinanceModule().saveFinanceTemplateFromVisibleForm(form);
  }

  function editFinanceTemplate(id) {
    return getFinanceModule().editFinanceTemplate(id);
  }

  function deleteFinanceTemplate(id) {
    return getFinanceModule().deleteFinanceTemplate(id);
  }

  function cloudSyncFinanceById(id) {
    return getFinanceModule().cloudSyncFinanceById(id);
  }
  ...
  function fillFinanceTemplate(templateId) {
    return getFinanceModule().fillFinanceTemplate(templateId);
  }
  ...
  function scheduleCloudAutosync(source = 'save') {
    ...
    const attemptAutosync = () => {
      ...
      runWhenMainThreadFree(() => {
        runCloudAutosyncNow(false).catch((error) => console.warn('Cloud autosync deferred run failed', error));
      }, { timeout: 10000 });
    };
    cloudAutosyncTimer = window.setTimeout(attemptAutosync, baseDelay);
  }
  ...
  // Mapa Supabase tabulek na cílené loader klíče. Více tabulek stejného modulu
  // se mapuje na stejný klíč, takže v jednom debounce okně po dedup přes Set
  // spustíme každý loader nejvýš jednou. Neznámý zdroj = fallback na full reload.
  const REALTIME_TABLE_LOADERS = {
    shopping_lists: ['shopping'],
    shopping_list_items: ['shopping'],
    profiles: ['profiles'],
    household_members: ['households', 'profiles'],
    contracts: ['contracts'],
  ...
  function collectRealtimeLoaderKeys(sources) {
    ...
    sources.forEach((source) => {
      const mapping = REALTIME_TABLE_LOADERS[source];
      if (!mapping) {
        hasUnknown = true;
        return;
      }
      mapping.forEach((key) => keys.add(key));
    });
    return { keys: [...keys], hasUnknown };
  }

  function describeRealtimeSources(sources) {
    const arr = [...sources];
    if (!arr.length) return 'cloud';
    if (arr.length <= 3) return arr.join(', ');
    return `${arr.length} zdrojů`;
  }

  async function runCloudRealtimeRefresh(sources) {
    const { keys, hasUnknown } = collectRealtimeLoaderKeys(sources);
    if (hasUnknown || !keys.length) {
      // Neznámý source nebo žádné mapování — bezpečný full reload,
      // aby data nezůstala zatuhlá pro tabulku, kterou zatím nemapujeme.
      await cloudLoadAllModules(false, { skipRealtimeSetup: true });
      return;
    }
    for (const key of keys) {
      await runRealtimeLoaderByKey(key);
    }
  }
  ...
  function handleAction(button) {
    ...
    if (action === 'confirm-fuelio-import') {
      confirmFuelioImport({ syncCloud: cloudReady() });
      return;
    }
    ...
    if (action === 'prompt-add-shopping-list') {
    ...
    if (action === 'shopping-qty-inc') {
      updateShoppingQuantity(button.dataset.id || '', 1);
      return;
    }
    if (action === 'shopping-qty-dec') {
      updateShoppingQuantity(button.dataset.id || '', -1);
      return;
    }
    if (action === 'open-shopping-done-modal') {
      getShoppingActions().setDoneModalOpen(true);
      return;
    }
    if (action === 'close-shopping-done-modal') {
      getShoppingActions().setDoneModalOpen(false);
      return;
    }
    ...
    if (action === 'select-reading-entry-meter') {
    ...
    if (action === 'finance-account-edit') {
      setFinanceAccountEdit(button.dataset.id);
      return;
    }
    ...
    if (action === 'delete-finance-template') {
      deleteFinanceTemplate(button.dataset.id);
      return;
    }
    ...
    if (action === 'subscription-month-current') {
      setSubscriptionMonth(todayISO().slice(0, 7));
      return;
    }
    if (action === 'subscription-month-next') {
      shiftSubscriptionMonth(1);
      return;
    }
    if (action === 'subscription-filter') {
      setSubscriptionPaymentFilter(button.dataset.filter || 'all');
      return;
    }
    if (action === 'subscription-toggle-paid') {
      toggleSubscriptionPaid(button.dataset.subscriptionId, button.dataset.personId, button.dataset.month || subscriptionSelectedMonth());
      return;
    }
    ...
    if (action === 'delete-subscription-person') {
      deleteSubscriptionPerson(button.dataset.id);
      return;
    }
    if (action === 'delete-subscription-share') {
      deleteSubscriptionShare(button.dataset.subscriptionId, button.dataset.personId);
      return;
    }
    if (action === 'delete-subscription-payment') {
      deleteSubscriptionPayment(button.dataset.id);
      return;
    }
  ...
  // PWA runtime — vše (diagnostika, install prompt, cache clear, update flow,
  // service worker registrace, iOS controllerchange guard) přesunuto do pwa.js.
  // Tady zůstávají jen tenké wrappery pro action handlery a boot.
  function promptInstallApp() { return getPwaModule().promptInstallApp(); }
  function runPwaDiagnostics() { return getPwaModule().runPwaDiagnostics(); }
  ...
  app.addEventListener('pointerdown', (event) => {
    const row = event.target.closest('.shopping-listonic-item.done[data-shopping-row-id]');
    if (!row || event.target.closest('[data-action]')) return;
    shoppingSwipeStart = { id: row.dataset.shoppingRowId, x: event.clientX, y: event.clientY };
  });

  app.addEventListener('pointerup', (event) => {
    if (!shoppingSwipeStart) return;
    const start = shoppingSwipeStart;
    shoppingSwipeStart = null;
    const dx = event.clientX - start.x;
    const dy = Math.abs(event.clientY - start.y);
    if (dx < -58 && dy < 45) deleteDoneShoppingBySwipe(start.id);
  });
  ...
  app.addEventListener('change', (event) => {
    ...
    const readingsPricingModeControl = event.target.closest('form[data-form="add-reading-meter"] select[name="pricingMode"], form[data-form="update-reading-meter"] select[name="pricingMode"]');
    if (readingsPricingModeControl) {
    ...
    const readingsDetailFilter = event.target.closest('[data-reading-detail-filter]');
    if (readingsDetailFilter) {
      const type = readingsDetailFilter.dataset.readingDetailFilter;
      if (type === 'meter') readingsDetailMeterId = readingsDetailFilter.value || '';
      if (type === 'period') readingsDetailPeriod = readingsDetailFilter.value || '12';
      if (type === 'compare') {
    ...
    const garageOverviewVehicle = event.target.closest('[data-garage-overview-vehicle]');
    if (garageOverviewVehicle) {
    ...
    const garageCalcVehicle = event.target.closest('[data-garage-calc-vehicle]');
    if (garageCalcVehicle) {
