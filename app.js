(() => {
  ...
  const APP_VERSION = 'Domácnost+ v.0.1_323';
  const APP_BUILD = 323;
  ...
  function renderNextPlanCard() {
    const steps = [
      { title: 'Domácnost+ v.0.1_288', note: 'Oprava mezery pod Home panely: nalezen dvojitý padding (.app-frame měl vlastní padding-bottom, který se sčítal s app-shell). V home módu drží rezervu pro lištu jen app-shell, vnitřní kontejnery mají padding 0. Finální mobilní override (poslední v souboru) sjednocuje konkurující @media pravidla → hero se roztáhne na celou výšku až k liště.' },
      { title: 'Domácnost+ v.0.1_287', note: 'Home doladění: hero karta se nyní dotáhne těsně ke spodní liště, text v panelech se neořezává v půlce řádku – počet řádků se přizpůsobí výšce buňky (3 řádky u mála panelů, 2 u 9–10).' },
      ...
      { title: 'Domácnost+ v.0.1_283', note: 'FIX: Home hero karta nyní správně zaplňuje celou výšku obrazovky ve fullscreen módu. Opravena chybějící CSS pravidla pro count-9 a count-10 panelů (byly ořezány "neviditelnou stěnou"). Přebita media-query max-height omezení která blokovala flex layout v app-shell módu.' },
      { title: 'Domácnost+ v.0.1_282', note: 'Opravy UI: Home nepůjde scrollovat, ikony na panelech větší, popis na panelech až 3 řádky, z HDO přehledu odebrána redundantní statistická okna, bílý text má všude tmavší pozadí, výrazněji označení dlužníků v Předplatném.' },
      ...
      { title: 'Domácnost+ v.0.1_130', note: 'Hotovo: nový modul Předplatné pro streamovací služby, sdílení s lidmi, měsíční zaškrtávání plateb, dluhy a platby dopředu.' },
      { title: 'Domácnost+ v.0.1_129', note: 'Hotfix: Home panel Záruky už bezpečně pracuje i bez nejbližší záruky nebo bez vypočtených dnů, takže nepadá na currentWarranty.days.' },
    ...
    return `
      <article class="reading-meter-card reading-meter-${escapeHtml(meta.className)} ${meter.archived ? 'muted-item' : ''} ${isEditing ? 'is-editing' : ''}">
        ...
        ${isEditing ? `<div class="action-details compact-edit-details readings-form-drawer reading-meter-edit-drawer open">
          <div class="details-like-head"><span>Upravit měřidlo</span><em>název, typ, jednotka, ceny</em></div>
    ...
    if (activeTab === 'overview') {
      ...
      content = `
        <section class="card desktop-span-2 readings-panel panel-overview">
          <div class="card-header"><div><h2>Odečty</h2><p>Elektřina, plyn a voda v jednom přehledu. U dvoutarifní elektřiny se v souhrnu počítá T1 + T2 dohromady, detail tarifů zůstává vidět v textu.</p></div><span class="badge ${cloudReady() ? 'good' : ''}">${cloudReady() ? 'cloud domácnost' : 'lokálně'}</span></div>
